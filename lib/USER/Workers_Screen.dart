import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:thzhilali/CONSTANTS/my%20widget.dart';
import 'package:thzhilali/PROVIDER/homeprovider.dart';
import 'package:thzhilali/USER/Jobtype_Screen.dart';
import 'package:thzhilali/USER/Viewmore_Screen.dart';

import '../CONSTANTS/my colors.dart';
import 'Bottom_screen.dart';

class Workers extends StatelessWidget {
  const Workers({super.key});

  @override
  Widget build(BuildContext context) {
    var height = MediaQuery.of(context).size.height;
    var width = MediaQuery.of(context).size.width;
    return Scaffold(
      backgroundColor: green,
      appBar: PreferredSize(
          preferredSize: Size.fromHeight(75),
          child: AppBar(
            backgroundColor: green,
            leading: GestureDetector(
                onTap: () {

                },
                child: Icon(
                  Icons.arrow_back_ios,
                  color: white,
                  size: 19,
                )),
            centerTitle: true,
            title: Text(
              "Workers",
              style:
                  TextStyle(fontSize: 16, fontFamily: "semibold", color: white),
            ),
          )),
      body: Container(
        height: height,
        width: width / 1,
        decoration: BoxDecoration(
          color: white,
          borderRadius: BorderRadius.only(
              topLeft: Radius.circular(30), topRight: Radius.circular(30)),
        ),
        child: SingleChildScrollView(
          child: Consumer<HomeProvider>(
            builder: (context,value,child) {
              return Column(
                children: [
                  SizedBox(
                    height: 15,
                  ),
                 value.RegistrationList.isNotEmpty? Workersearcch(TextInputType.text, green, "Search Your Worker", Colors.grey, Icons.search, 20, height/16, width/1.1
                 ):
                  Center(child: Padding(
                    padding: const EdgeInsets.only(top:225),
                    child: Column(
                     children: [
                       Text("No workers in this location, try another"),
                       Text("location to find workers"),
                     ],
                    ),
                  )),
                  ListView.builder(
                      itemCount: value.RegistrationList.length,
                      physics: ScrollPhysics(),
                      shrinkWrap: true,
                    itemBuilder:(context, index) {
                              var item=value.RegistrationList[index];
                        return InkWell(
                        onTap: () {
                        Navigator.push(context, MaterialPageRoute(builder: (context) => viewmore(Name: item.name,age: item.Age,Experience: item.Experience,
                        Location: item.Location,Mainimage: item.Image,Mobilenmbr: item.Phonenmbr,whatsappnmbr:item.Whatsappnmbr,Workerimage:item.WorkerImage,Worktype: item.Jobtype,id:item.id,),));
                        },
                        child:Container(
                          margin: EdgeInsets.symmetric(horizontal: 13,vertical: 13),
                    height: height / 4.2,
                    width: width / 1.1,
                    decoration: BoxDecoration(
                        color: green, borderRadius: BorderRadius.circular(11)),
                    child: Column(
                      children: [
                        SizedBox(
                          height: 8,
                        ),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceAround,
                          children: [
                            Text(
                              value.RegistrationList[index].Jobtype,
                              style: TextStyle(
                                  fontFamily: "semibold",
                                  fontSize: 13,
                                  color: white),
                            ),
                            Text(
                              "Available",
                              style: TextStyle(
                                  color: Colors.green,
                                  fontFamily: "semibold",
                                  fontSize: 11),
                            )
                          ],
                        ),
                        SizedBox(
                          height: 6,
                        ),
                        Divider(
                          height: 1,
                          endIndent: 18,
                          indent: 18,
                          color: Colors.grey,
                        ),
                        SizedBox(
                          height: 18,
                        ),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceAround,
                          children: [
                            CircleAvatar(
                              radius: 31,
                              backgroundImage:NetworkImage(value.RegistrationList[index].Image),
                            ),
                            Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                SizedBox(
                                  width: width/1.8,
                                  child: Row(
                                    mainAxisAlignment:
                                        MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text(
                                        value.RegistrationList[index].name,
                                        style: TextStyle(
                                            fontFamily: "montserratmedium",
                                            fontSize: 16,
                                            color: white),
                                      ),
                                      // SizedBox(width: 29,),
                                      Padding(
                                        padding: const EdgeInsets.only(right: 14),
                                        child: Icon(
                                          Icons.favorite,
                                          color: white,
                                          size: 28,
                                        ),
                                      )
                                    ],
                                  ),
                                ),
                                Padding(
                                  padding: const EdgeInsets.only(top: 28,left: 120),
                                  child: Container(
                                    height: height/21,
                                    width: width/4,
                                    decoration: BoxDecoration(
                                      color: Colors.green,
                                      borderRadius: BorderRadius.circular(10),
                                      boxShadow: [BoxShadow(color: Colors.black26,offset: Offset(5, 5),blurRadius: 10)]
                                    ),
                                    child: Center(
                                      child: GestureDetector(
                                        onTap: () {
                                          Navigator.push(context, MaterialPageRoute(builder: (context) => viewmore(Name: item.name,age: item.Age,Experience: item.Experience,
                      Location: item.Location,Mainimage: item.Image,Mobilenmbr: item.Phonenmbr,whatsappnmbr:item.Whatsappnmbr,Workerimage:item.WorkerImage,Worktype: item.Jobtype,id:item.id,),));
                                        },
                                        child: Text("View More",style: TextStyle(
                                          color: Colors.white70,fontSize: 12
                                        ),),
                                      ),
                                    ),
                                  ),
                                )
                              ],
                            )
                          ],
                        ),
                              ],
                            ),
                              )
                              );

                      },
                      ),
                ],
              );
            }
          )
      )
    )
    );
  }
}
